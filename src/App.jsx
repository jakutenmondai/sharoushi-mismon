...
function detectPtns(text) {
  return PTNS.filter(p => p.kw.some(kw => (text||"").includes(kw))).map(p => p.id);
}
function detectRev(text) {
  return REV_KW.some(kw => (text||"").includes(kw));
}
...
function loadLocal() {
  try { return JSON.parse(localStorage.getItem(SK)||"null")||{qs:[],books:[],userId:null}; }
  catch { return {qs:[],books:[],userId:null}; }
}
...
・過去57回の出題実績
・難易度ランク判定（G/A/B/C/D/E/Fで答えてください。最後に「RANK:X」の形式で必ず記載）
...
function extractRank(text) {
  const m = text.match(/RANK:([GABCDEF])/);
  return m ? m[1] : "B";
}

async function aiExplain(form, books, pdfMap) {
  const subj = SUBJS.find(s=>s.full===form.subject||s.short===form.subject);
  const subjLabel = subj ? `${subj.full}（${subj.short}）` : form.subject;
  const book = books.find(b=>b.id===form.bookId);
  const pdfKey = form.bookId||"__tmp";
  const pdfB64 = pdfMap[pdfKey]?.b64;

  let content;
  if(form.inputMode==="pdf" && pdfB64) {
    const pageHint = form.pageRange ? `${form.pageRange}ページを解説` : `問題番号「${form.nendoNo||form.qNo}」を見つけて解説`;
    content = [{type:"document",source:{type:"base64",media_type:"application/pdf",data:pdfB64}},
      {type:"text",text:`このPDFは「${book?.name||"問題集"}」。科目:${subjLabel}\n${pageHint}。問題文も引用してください。`}];
  } else if(form.inputMode==="image") {
    return null;
  } else if(form.inputMode==="text" && form.questionText) {
    content = `【科目】${subjLabel}\n【年度番号】${form.nendoNo||"未入力"}\n【問題文】\n${form.questionText}`;
...
async function aiImage(b64, mime, subject) {
  ...
  return callAI([{role:"user",content:[
    ...
    {type:"text",text:`この社労士試験問題（1ページ・5肢一択・複数問OK）を全問まとめて解説。科目:${subj?.full||subject}`}
  ]}]);
}

async function aiBatch(text) {
  return callAI([{role:"user",content:
    `以下の社労士試験問題をまとめて解説してください。各問を【問題N】で区切り、引っかけパターン・正解根拠・対策を解説。\n\n${text}`}]);
}

// ── Supabase CRUD ──
async function sbUpsertShared(q) {
  if(!sb) return;
  const { error } = await sb.from("shared_questions").upsert({
    id: q.nendoNo || q.id.toString(),
    subject: q.subject,
    nendo_no: q.nendoNo,
...
async function sbFetchShared(nendoNo) {
  if(!sb || !nendoNo) return null;
  const { data, error } = await sb.from("shared_questions").select("*").eq("nendo_no", nendoNo).single();
  if(error) return null;
  return data;
}

async function sbFetchAllShared() {
  if(!sb) return [];
  const { data } = await sb.from("shared_questions").select("*").order("updated_at",{ascending:false});
  return data||[];
}
...
export default function App() {
  ...
  const [cd, setCd] = useState(daysLeft);
  ...
  const [sbStatus, setSbStatus] = useState(sb?"接続中":"未接続");
  const imgRef = useRef();
  ...
  useEffect(()=>{
    if(!st.userId) {
      const uid = genUserId();
      setSt(prev=>{ const next={...prev,userId:uid}; saveLocal(next); return next; });
    }
  },[]);

  useEffect(()=>{ const id=setInterval(()=>setCd(daysLeft()),60000); return ()=>clearInterval(id); },[]);
  ...
  const newForm = () => setForm({
    ...
    missCount:1, memo:"", tags:"", explanation:"",
    ptns:[], hasRev:false,
  ...
  // Supabaseから問題を取得
  const fetchFromSupabase = async nendoNo => {
    if(!nendoNo || !sb) return null;
    if(sharedCache[nendoNo]) return sharedCache[nendoNo];
    const data = await sbFetchShared(nendoNo);
    if(data) { setSharedCache(c=>({...c,[nendoNo]:data})); return data; }
    return null;
  };
  ...
  const runAI = async () => {
    ...
    try {
      // まずSupabaseに同じ年度番号のデータがあるか確認
      if(form.nendoNo && sb) {
        const shared = await fetchFromSupabase(form.nendoNo);
      ...
      <div style={{background:"linear-gradient(135deg,#0f2744,#1a3a72)",padding:"12px 16px"}}>
        <div style={{maxWidth:1200,margin:"0 auto",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
          ...
          <div style={{background:"rgba(255,255,255,.1)",borderRadius:10,padding:"8px 16px",textAlign:"center"}}>
            ...
            <div style={{fontSize:9,color:"#e0f2fe",marginBottom:4}}>{fmtDate(cd.exam)}</div>
            <div style={{fontSize:28,fontWeight:900,color:"#f59e0b",lineHeight:1}}>{cd.days}<span style={{fontSize:12,color:"#fde68a",marginLeft:2}}>日</span></div>
          </div>
          <div style={{marginLeft:"auto",display:"flex",gap:14,flexWrap:"wrap"}}>
            ...
            {sb&&<div style={{fontSize:9,color:"#86efac",alignSelf:"center"}}>{sbStatus}</div>}
          </div>
        </div>
      </div>
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:"14px 12px"}}>
        ...
        {page==="add" && form && (
          <AddPage form={form} setForm={setForm} books={st.books}
            ...
            onRunAI={runAI} onSave={()=>saveQ(form)}
        ...
        {page==="shared" && <SharedPage sb={sb} fetchAll={sbFetchAllShared} />}
      </div>
    ...
    <div style={C.panel}>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
        <select value={filter.subj} onChange={e=>sf({subj:e.target.value})} style={C.sel}>
          ...
          {SUBJS.map(s=><option key={s.full} value={s.full}>{s.short} {s.full}</option>)}
        </select>
    ...
    <div className="card" onClick={onClick} style={{borderRadius:9,padding:"10px 12px",cursor:"pointer",background:bg,borderLeft:`4px solid ${acc}`,outline:selected?"2px solid #3b82f6":"none",boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
        <div style={{display:"flex",gap:4,alignItems:"center",flexWrap:"wrap"}}>
          <span style={{fontSize:8,background:"#0f2744",color:"#fff",borderRadius:4,padding:"1px 5px",fontWeight:700}}>{subj?.short||q.subject?.slice(0,3)}</span>
    ...
    <div style={C.panel}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
        <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
          <span style={{fontSize:9,background:"#0f2744",color:"#fff",borderRadius:5,padding:"2px 7px",fontWeight:700}}>{subj?.short||q.subject?.slice(0,3)} {subj?.full||q.subject}</span>
      ...
      <div style={{marginBottom:10}}>
        ...
        {book&&<div style={{fontSize:10,color:"#64748b",marginBottom:3}}>📚 {book.name}</div>}
        {last&&<div style={{fontSize:10,color:"#94a3b8"}}>最後に解いたのは <strong>{last.date}</strong>（{last.r}）</div>}
      </div>
      {pats.length>0&&(
        <div style={{marginBottom:10,padding:"8px 10px",background:"#fff7ed",borderRadius:8,border:"1px solid #fed7aa"}}>
          ...
          <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
            {pats.map(p=><div key={p.id} title={p.tip} style={{background:p.bg,border:`1.5px solid ${p.c}`,color:p.c,borderRadius:7,padding:"3px 8px",fontSize:9,fontWeight:700,cursor:"help"}}><sup>{p.sym}</sup>{p.lbl} <span style={{fontWeight:400,opacity:.8}}>→{p.tip}</span></div>)}
          </div>
        </div>
      )}
...
function AddPage({form,setForm,books,aiBusy,aiOut,aiErr,pdfMap,setPdfMap,imgRef,onRunAI,onSave,onCancel,onAddBook,onDeleteBook}){
  ...
  const subj=SUBJS.find(s=>s.full===form.subject);

  const handlePdf=e=>{
    const file=e.target.files?.[0]; if(!file) return;
    const bookId=form.bookId||"__tmp";
    const r=new FileReader();
    r.onload=ev=>setPdfMap(m=>({...m,[bookId]:{b64:ev.target.result.split(",")[1],name:file.name}}));
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
          ...
          <Sec title="📚 科目・番号">
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <div>
                ...
                <div style={{fontSize:9,color:"#94a3b8",marginTop:2}}>年度-問番号-選択肢</div>
              </div>
              <div>
                <L>問題集番号</L>
                <input style={C.inp} placeholder="例: No.234" value={form.qNo} onChange={e=>s("qNo",e.target.value)}/>
                <div style={{fontSize:9,color:"#94a3b8",marginTop:2}}>各自の問題集の番号</div>
              </div>
            </div>
            <L>問題集を紐付け</L>
            ...
            <div style={{display:"flex",gap:5,marginTop:5}}>
              <input style={{...C.inp,flex:1}} placeholder="問題集名を追加…" value={newBook} onChange={e=>setNewBook(e.target.value)}
                onKeyDown={e=>{ if(e.key==="Enter"&&newBook.trim()){onAddBook({id:`b_${Date.now()}`,name:newBook.trim()});setNewBook("");}}}/>
              <button onClick={()=>{ if(newBook.trim()){onAddBook({id:`b_${Date.now()}`,name:newBook.trim()});setNewBook("");}}} style={{...C.btnP,padding:"6px 10px"}}>＋</button>
            </div>
            {books.length>0&&(
              <div style={{marginTop:5,display:"flex",gap:4,flexWrap:"wrap"}}>
                {books.map(b=>(
                  <div key={b.id} style={{fontSize:9,background:"#f1f5f9",borderRadius:5,padding:"2px 6px",display:"flex",alignItems:"center",gap:4}}>
                    {b.name}
        ...
        <div>
          <Sec title="📝 入力方法">
            ...
            {form.inputMode==="url"&&(
              <><L>URL</L><input style={C.inp} placeholder="https://..." value={form.sourceUrl} onChange={e=>s("sourceUrl",e.target.value)}/></>
            )}
            {(form.inputMode==="pdf"||form.inputMode==="number")&&(
              <div style={{marginTop:8}}>
                <L>PDFをアップ（複数の問題集も個別にアップ可）</L>
                {Object.entries(pdfMap).map(([key,val])=>(
                  <div key={key} style={{background:"#f0fdf4",border:"1px solid #86efac",borderRadius:7,padding:"6px 10px",fontSize:10,color:"#166534",display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
                    📄 {val.name}
                ...
                <L>ページ指定（任意）</L>
                <input style={C.inp} placeholder="例: 15〜20ページ、または問題番号" value={form.pageRange||""} onChange={e=>s("pageRange",e.target.value)}/>
              </div>
            )}
          ...
          <Sec title="🤖 AI解説">
            ...
            {aiOut&&(
              <div style={{background:"#f0f9ff",border:"1px solid #bae6fd",borderRadius:9,padding:11,marginBottom:8,maxHeight:240,overflowY:"auto"}}>
                <div style={{fontSize:10,fontWeight:900,color:"#0369a1",marginBottom:5}}>✨ プレビュー（ランク自動判定済み）</div>
                <pre style={{fontSize:11,whiteSpace:"pre-wrap",fontFamily:"inherit",lineHeight:1.8}}>{aiOut.replace(/RANK:[GABCDEF]/g,"")}</pre>
              </div>
            )}
            <L>解説テキスト（手動編集可）</L>
            <textarea style={{...C.inp,height:110,resize:"vertical"}} placeholder="AI解説がここに入ります。自由に編集できます。" value={form.explanation||""} onChange={e=>s("explanation",e.target.value)}/>
          </Sec>
        </div>
      </div>
  ...
  return(
    <div style={C.panel}>
      ...
      <div style={{fontSize:11,color:"#64748b",marginBottom:12}}>
        複数問・5肢一択・1ページをまとめて貼り付けてAI解説。スクショや画像も対応。<br/>
        「R06-01AとR06-01Bの解説を教えて」のように年度番号を指定してもOK。
      </div>
      <textarea style={{...C.inp,height:180,resize:"vertical",marginBottom:10,width:"100%"}}
        placeholder={`例1: 問題文をそのまま貼り付け（複数問OK）\n\n例2: R06-01AとH28-1イの解説を教えて\n\n例3: 労働基準法の解雇予告について、間違えやすいポイントをまとめて`}
...
// ── SHARED PAGE ──
function SharedPage({sb,fetchAll}){
  const [items,setItems]=useState([]);
  const [loading,setLoading]=useState(true);
  useEffect(()=>{
    if(!sb){setLoading(false);return;}
    fetchAll().then(d=>{ setItems(d); setLoading(false); });
  },[]);
  if(!sb) return(
    <div style={C.panel}>
      <div style={{fontSize:15,fontWeight:900,color:"#0f2744",marginBottom:8}}>🌐 共有問題データベース</div>
      <div style={{background:"#fef2f2",border:"1px solid #fca5a5",borderRadius:8,padding:14,fontSize:12,color:"#dc2626"}}>
        Supabaseが設定されていません。.envファイルにAPIキーを設定してください。
      </div>
    </div>
  );
  return(
    <div style={C.panel}>
      ...
      <div style={{fontSize:11,color:"#64748b",marginBottom:12}}>誰かが登録した問題文・AI解説が自動で共有されます。年度番号で検索できます。</div>
      {loading?<div style={{textAlign:"center",padding:"20px",color:"#94a3b8"}}>読み込み中…</div>:(
        items.length===0?<div style={{textAlign:"center",padding:"20px",color:"#94a3b8"}}>まだ共有データはありません。問題を登録して共有しましょう！</div>
          ...
          {items.map(item=>{
            ...
            return(
              <div key={item.id} style={{background:"#f8fafc",borderRadius:8,padding:"10px 12px",border:"1px solid #e2e8f0"}}>
                ...
                {item.question_text&&<div style={{fontSize:11,color:"#1e293b",marginBottom:4,lineHeight:1.7}}>{item.question_text.slice(0,100)}{item.question_text.length>100?"…":""}</div>}
                {item.explanation&&<div style={{fontSize:10,color:"#64748b"}}>✨ AI解説あり</div>}
              </div>
            );
          })}
        </div>
